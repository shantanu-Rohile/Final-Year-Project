    ...
    >
      ...
      <div className="bg-ter px-3 py-2 text-sm font-medium txt">
        <div className="flex items-center justify-between">
          <span className="text-[var(--btn)]">{theme.label}</span>
          {isSelected && <Check size={14} className="text-[var(--txt)]" />}
        </div>
      </div>
...
export default function Themes() {
  ...
  const applyTheme = (id) => {
    ...
    // For light theme, remove all classes
    if (id === "light") {
      document.documentElement.classList.remove(...all);
    } else {
    ...
    <div className="max-w-6xl mx-auto space-y-12">
      <section className="space-y-6">
        <div className="flex items-center gap-3">
          <h2 className="text-2xl font-bold txt text-[var(--txt)]">Choose Your Theme</h2>
        </div>
